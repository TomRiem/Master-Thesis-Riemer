...
For the first numerical experiment, we use for $\rho_{\theta_e}(x, t)$ an feed-forward neural network, denoted by $fnn_{\theta_e}(x, t)$, with $L$-layers for each edge $e \in \mathcal{E}$, which is defined by 
\begin{gather}
    \label{one_for_each}
...
We note that using just one neural network for all edges is only possible because the graph $\Gamma$ is equilateral, see \cref{graph_assumptions}, and thus  is defined for $x \in [0,1]$ on every edge. % stimmt nicht so ganz, macht es aber um vieles einfacher. Man kann die gleichen Kollokationspunkte verwenden!
%Projektion
