...
\end{equation*} 
In other words, $L_2(\Gamma)$ is the orthogonal direct sum of spaces $L_2(e)$, i.e. $L_2(\Gamma) = \bigotimes_{e \in \mathcal{E}} L_2(e)$. Of course, both $L_2(e)$ and $L_2(\Gamma)$ are Hilbert spaces. \\
...
\begin{definition}[{\cite[Definition~1.4.1.]{BerkolaikoKuchment:2013}}]
    \label{quantum graph}
    Quantum graph is a metric graph equipped with a differential operator $\mathcal{H} \left[ \cdot \right]$ (Hamiltonian), accompanied by “appropriate” vertex conditions. That is, a quantum graph $\Gamma$ is a triple $(\text{metric graph } \Gamma, \; \text{Hamiltonian } \mathcal{H} \left[ \cdot \right], \; \text{vertex conditions})$.
\end{definition}
