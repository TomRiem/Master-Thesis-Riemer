...
function g(t::Float64)
    return ((t<0.2)+(t>0.4)*(t<0.6)+(t>0.8))
end
...
# Mass matrix
M = spdiagm(he[1] * ones(ne)) 

# Advection term
A = spdiagm(2 * ones(ne)) + spdiagm(-1 => -2 * ones(ne-1))
...
# Diffusion term
D = spdiagm(1000 * ones(ne)) + spdiagm(-1 => -500 * ones(ne-1)) + spdiagm(1 => -500 * ones(ne-1))
D[1,1] = 1/he
D[ne,ne] = 1/he
